...
export default function AdminOversight() {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            const [usersRes, subsRes] = await Promise.all([
                ...
                fetch(`${api}/submissions/admin/all?status=${statusFilter}`, { headers }),
            ]);
    ...
    const handleStatusUpdate = async () => {
        ...
        try {
            const token = await user.getIdToken();
            await fetch(`${process.env.NEXT_PUBLIC_API_URL}/auth/admin/users/${actionUser._id}/status`, {
                method: "PATCH",
            ...
            <DashboardLayout navItems={ADMIN_NAV} title="SEPMS Admin">
                ...
                <div className="grid gap-4 grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 mb-8">
                    {[
                        ...
                        { label: "Entrepreneurs", value: userStats.entrepreneurs || 0, icon: "🚀" },
                                    ...
                                    {loading ? (
                                        ...
                                        users.map((u) => (
                                            <TableRow key={u._id}>
                                                ...
                                                <TableCell>
                                                    <Badge variant={roleBadge(u.role)} className="text-xs capitalize">{u.role}</Badge>
                                                </TableCell>
                                                <TableCell>
                                                    <Badge variant={statusBadge(u.status)} className="text-xs capitalize">{u.status}</Badge>
                                                </TableCell>
                                                <TableCell className="text-sm text-muted-foreground">
                                                    {new Date(u.createdAt).toLocaleDateString()}
                                                </TableCell>
                                                <TableCell className="text-right">
                                                    <Button size="sm" variant="outline" onClick={() => { setActionUser(u); setNewStatus(u.status); }}>
                                                        Manage
                                                    </Button>
                                                </TableCell>
                                            </TableRow>
                                    ...
                                    {loading ? (
                                        ...
                                        submissions.map((s) => (
                                            <TableRow key={s._id}>
                                                ...
                                                <TableCell className="text-sm text-muted-foreground">
                                                    {(s.entrepreneurId as any)?.fullName || "—"}
                                                </TableCell>
                                                <TableCell className="text-sm capitalize">{s.sector?.replace("_", " ")}</TableCell>
                                                <TableCell className="text-sm">${s.targetAmount?.toLocaleString() || "—"}</TableCell>
                                                <TableCell>
                        ...
                        {actionUser && (
                            <div className="space-y-4">
                                <div className="space-y-2">
                                    <p className="text-sm"><span className="text-muted-foreground">Name:</span> {actionUser.fullName}</p>
                                    <p className="text-sm"><span className="text-muted-foreground">Email:</span> {actionUser.email}</p>
                                    <p className="text-sm"><span className="text-muted-foreground">Role:</span> <Badge variant={roleBadge(actionUser.role)} className="text-xs capitalize">{actionUser.role}</Badge></p>
                                </div>
                                <Separator />
                                <div className="space-y-2">
